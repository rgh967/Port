	...
	<div id="wrap">
		<div id="container">
			<div id="content">
				...
				<form class="result-filter" action="category.do" method="post">
					<!-- 카테고리 시작 -->
					<div class="finder">
						<dl class="dl_first">
							<dt>
				...
				<div class="recipes">
								...
								<li><img id="picture"
									...
									<div class="recipe_content">
										<span id="title">${recipe.RECIPE_TITLE}</span><br> <span
											...
											id="name">${recipe.member.MEMBER_NAME}</span>
									</div></li>
							</c:forEach>
						</ul>
					</div>
					...
					<div class="paging">
						...
						<c:if test="${page > 1}">
							<a class="prev" href="RecipeList.bo?page=${page-1}">이전</a>
						</c:if>
						<c:forEach var="a" begin="${startpage}" end="${endpage}">
							...
							<c:if test="${a != page}">
								<a class="current" href="RecipeList.bo?page=${a}">${a}</a>
							</c:if>
						</c:forEach>
						...
						<c:if test="${page < maxpage}">
							<a class="next" href="RecipeList.bo?page=${page+1}">다음</a>
						</c:if>
					</div>
				</div>
			</div>
